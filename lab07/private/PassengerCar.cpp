...
PassengerCar::PassengerCar(std::string brand, int powerHp, double weight, int passengerSeats, std::string bodyType)
    : Vehicle(std::move(brand), powerHp, weight),
     passengerSeats_(passengerSeats),
     bodyType_(std::move(bodyType)), 
     totalTrips_(0) {}

int PassengerCar::passengerSeats() const { 
    return passengerSeats_; 
}
const std::string& PassengerCar::bodyType() const { 
    return bodyType_;
}
int PassengerCar::totalTrips() const {
    return totalTrips_; 
}
...
std::string PassengerCar::category() const { 
    return "PassengerCar"; 
}
